...
char hostName[16 + 1];
// extern uint8_t COIL_REG[4];
extern DIGITAL_OUTPUTS digital_outputs;
...
static TCPIP_HTTP_NET_IO_RESULT
HTTPPostLCD(TCPIP_HTTP_NET_CONN_HANDLE connHandle);
#endif
...
TCPIP_HTTP_NET_IO_RESULT TCPIP_HTTP_NET_ConnectionGetExecute(TCPIP_HTTP_NET_CONN_HANDLE connHandle, const TCPIP_HTTP_NET_USER_CALLBACK *pCBack)
{
    ...
    if (!memcmp(filename, "digital-outputs.html", 19))
    {
        ...
        if (ptr)
        {
            if (*ptr == '1')
            {
                // SYS_CONSOLE_MESSAGE("on output 1\r\n");
                outputs(0, 1);
            }
        }
        else
        {
            // SYS_CONSOLE_MESSAGE("off output 1\r\n");
            outputs(0, 0);
        }
...
uint8_t TCPIP_HTTP_NET_ConnectionUserAuthenticate(TCPIP_HTTP_NET_CONN_HANDLE connHandle, const char *cUser, const char *cPass, const TCPIP_HTTP_NET_USER_CALLBACK *pCBack)
{
    if (strcmp(cUser, (const char *)"admin") == 0 && strcmp(cPass, (const char *)"microchip") == 0)
...
{
    SYS_CONSOLE_MESSAGE("Read inputsr\r\n");
...
TCPIP_HTTP_DYN_PRINT_RES TCPIP_HTTP_Print_out(TCPIP_HTTP_NET_CONN_HANDLE connHandle, const TCPIP_HTTP_DYN_VAR_DCPT *vDcpt)
{
    ...
    {
    case 0:
        if (((digital_outputs.buffer >> 0) & 1U) == 0)
            checked = false;
        else
            checked = true;
        break;
    case 1:
        if (((digital_outputs.buffer >> 2) & 1U) == 0)
            checked = false;
        else
            checked = true;
        break;
    case 2:
        if (((digital_outputs.buffer >> 3) & 1U) == 0)
            checked = false;
        else
            checked = true;
        break;
    case 3:
        if (((digital_outputs.buffer >> 4) & 1U) == 0)
            checked = false;
        else
            checked = true;
        break;
